...
local function clearExpiredMutation(state)
	if state.activeMutation and os.time() >= (state.mutationExpiresAt or 0) then
...
local function applyLoadedData(state, data)
	...
	if typeof(data.activeMutation) == "string" and Mutations.getDefinition(data.activeMutation) then
		state.activeMutation = data.activeMutation
	end
	if typeof(data.mutationExpiresAt) == "number" then
		state.mutationExpiresAt = math.max(0, data.mutationExpiresAt)
	end
...
local function serializeState(state)
	...
	return {
		...
		mutationExpiresAt = state.mutationExpiresAt,
...
local function maybeStartMutation(player, state)
	if state.activeMutation then
		return false
	end
	if rng:NextNumber() > Mutations.TriggerChance then
		return false
	end
	local mutationId = Mutations.roll(rng)
...
function StateService.getMutationMoneyMultiplierFromState(state)
	local mutation = getMutationDefinition(state)
	return mutation and mutation.moneyMultiplier or 1
end

function StateService.getMutationXpMultiplierFromState(state)
	local mutation = getMutationDefinition(state)
...
function StateService.getMoneyMultiplierFromState(state)
	return StateService.getPrestigeMultiplierFromState(state)
		...
		* StateService.getMutationMoneyMultiplierFromState(state)
end
...
function StateService.getXpMultiplierFromState(state)
	return StateService.getPrestigeMultiplierFromState(state)
		...
		* StateService.getMutationXpMultiplierFromState(state)
end
...
function StateService.getMoneyPerSecondFromState(state)
	return Balance.getMoneyPerSecond(state.fridgeLevel, {
		...
		mutation = StateService.getMutationMoneyMultiplierFromState(state),
	})
end
...
local function toPublicState(state)
	...
	return {
		...
		activeMutation = mutation and {
			id = state.activeMutation,
			name = mutation.displayName,
			description = mutation.description,
			expiresAt = state.mutationExpiresAt,
			moneyMultiplier = mutation.moneyMultiplier,
			xpMultiplier = mutation.xpMultiplier,
		} or nil,
		multipliers = {
			...
			prestige = StateService.getPrestigeMultiplierFromState(state),
			permanent = StateService.getPermanentMultiplierFromState(state),
			mutationMoney = StateService.getMutationMoneyMultiplierFromState(state),
			mutationXp = StateService.getMutationXpMultiplierFromState(state),
		},
	}
end
...
function StateService.savePlayer(player, force)
	...
	if ok then
		dirty[player] = nil
		print("[StateService] Saved player", player.UserId, "level", state.fridgeLevel, "xp", math.floor(state.fridgeXp), "food", #state.inventory, "skills", data.skillCount, "mutation", state.activeMutation or "none")
	else
		dirty[player] = true
...
function StateService.tickMoney(deltaTime)
	local shouldCheckMutations = os.clock() - lastMutationRoll >= Mutations.CheckIntervalSeconds
	if shouldCheckMutations then
		lastMutationRoll = os.clock()
	end

	for player, state in pairs(states) do
		local changed = clearExpiredMutation(state)
		if shouldCheckMutations and maybeStartMutation(player, state) then
			changed = true
		end
...
local function loadPlayer(player)
	...
	if data then
		...
		print("[StateService] Loaded player", player.UserId, "level", state.fridgeLevel, "xp", math.floor(state.fridgeXp), "food", #state.inventory, "skills", countUpgrades(state.upgrades), "mutation", state.activeMutation or "none")
	else
